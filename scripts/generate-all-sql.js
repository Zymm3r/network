...
async function run() {
  ...
  console.log(`Found ${products.length} products. Generating SQL...`);
  ...
  for (const p of products) {
    ...
    const th = generateTemplate(p.name, type, 'th').replace(/'/g, "''");
    const en = generateTemplate(p.name, type, 'en').replace(/'/g, "''");
  ...
  fs.writeFileSync(path.join(process.cwd(), 'scripts', 'upsert_all_products.sql'), sql);
  console.log('SQL generated and saved to scripts/upsert_all_products.sql');
}
