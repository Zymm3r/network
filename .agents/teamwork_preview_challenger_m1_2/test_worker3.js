...
class CappedStdout(io.StringIO):
    ...
    def write(self, s):
        ...
        if self.bytes_written > 50000 or self.lines_written > 1000:
            # THIS IS A VULNERABILITY: It writes the potentially HUGE string before raising.
            super().write(s)
...
async function main() {
    await runTest(
        "def memory_bomb():\n  print('A' * 10000000)", // 10MB string
        [{ input: "memory_bomb()", expected: "A" }],
        "Memory Bomb OOM Attack"
    );
}
