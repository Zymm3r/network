...
async function main() {
  ...
  // Test Case 1: Infinite loop with print
  const code1 = `
...
`;
  ...
  console.log("Running code 1 (infinite print)...");
  try {
    const resultsJson1 = await pyodide.runPythonAsync(wrapperCode);
    console.log(JSON.parse(resultsJson1)[0].actual);
  } catch (err) {
    console.error("Code 1 failed:", err);
  }
}
