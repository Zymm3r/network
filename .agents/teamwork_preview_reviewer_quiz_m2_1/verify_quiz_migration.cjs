...
const rawUpdateCount = (content.match(/UPDATE\s+public\.lessons/gi) || []).length;
console.log(`Raw UPDATE statements in file: ${rawUpdateCount}`);

if (rawUpdateCount !== matchCount) {
  errors.push(`Mismatch: found ${rawUpdateCount} raw UPDATE statements in file but only matched ${matchCount}. Some UPDATE statements might have syntax/formatting issues.`);
}
