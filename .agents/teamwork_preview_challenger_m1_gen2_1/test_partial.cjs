...
`;
    ...
    // Test 6: Function throwing output limit inside test case
    const code6 = `
def massive():
    print("A" * 60000)
    return "Done"
`;
    const testCases6 = [{ input: "massive()", expected: "Done" }];
    
    pyodide.globals.set('__USER_CODE__', code6);
    pyodide.globals.set('__TEST_CASES__', pyodide.toPy(testCases6));

    console.log("\\nRunning Test 6 (Massive print inside test case):");
    try {
        const resultsJson6 = await pyodide.runPythonAsync(wrapperCode);
        const res = JSON.parse(resultsJson6);
        console.log("Actual output ends with:", res[0].actual.substring(res[0].actual.length - 100));
        console.log("Passed:", res[0].passed);
    } catch (e) {
        console.error("Test 6 crashed pyodide? ", e.message);
    }
}
